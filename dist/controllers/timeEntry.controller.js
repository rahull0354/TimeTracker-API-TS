...
export const stopTimeEntry = async (req, res) => {
    try {
        ...
        if (timeEntry.status === "running") {
            ...
            const sessionDuration = Math.floor((sessionEntTime.getTime() - timeEntry.startTime.getTime()) / 60000);
...
export const applyBreakToTimer = async (req, res) => {
    try {
        const userId = req.user.id;
        const { timeEntryId } = req.params;
        const timeEntry = await TimeEntry.findOne({
        ...
        const sessionDuration = Math.floor((sessionEndTime.getTime() - timeEntry.startTime.getTime()) / 60000);
        timeEntry.sessions.push({
            type: "work",
            startTime: timeEntry.startTime,
...
export const completeTimeEntry = async (req, res) => {
    try {
        const userId = req.user.id;
        const { timeEntryId } = req.params;
        const timeEntry = await TimeEntry.findOne({
