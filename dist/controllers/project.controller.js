...
export const updateProjectDetails = async (req, res) => {
    try {
        ...
        const updatedProject = await Project.findByIdAndUpdate(projectId, { projectName, description, clientName, hourlyRate }, { new: true });
        res.status(200).json({
...
export const getAllProjectOfParticularUser = async (req, res) => {
    try {
        const userId = req.user.id;
...
export const getParticularProjectDetails = async (req, res) => {
    try {
        const userId = req.user.id;
        const { projectId } = req.params;
...
export const changeStatusOfProject = async (req, res) => {
    try {
        ...
        const updateStatus = await Project.findByIdAndUpdate(projectId, { status }, { new: true });
        res.status(200).json({
