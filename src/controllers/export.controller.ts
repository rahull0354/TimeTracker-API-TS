...
export const exportProjectsExcel = async (req: Request, res: Response) => {
  try {
    ...
    const { buffer, filename, mimetype } = formatXLSX(
      data,
      "Projects",
      "projects",
    );
...
export const exportTimeEntriesCSV = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
...
export const exportTimeEntriesExcel = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
...
export const exportProjectTimeEntryCSV = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    // 1. Fetching data
    const data = await fetchTimeEntriesOfAProject(userId, projectId as any);

    // 2. formatting the data to csv
    const projectName = data[0]?.["Project Name"] || "project";
    const { csv, filename, mimetype } = formatCSV(
      data,
      `project_${projectName}_time_entries`,
    );
...
export const exportProjectTimeEntryExcel = async (
  req: Request,
  res: Response,
) => {
  try {
...
export const exportSummaryReportCSV = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
...
export const exportSummaryReportExcel = async (req: Request, res: Response) => {
  try {
    const userId = (req as any).user.id;
...
export const exportTimeEntryByDateExcel = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    // 1. fetching the data
    const data = await fetchSpecificEntries(
      userId,
      startDate as any,
      endDate as any,
    );
